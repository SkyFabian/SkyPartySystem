    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("sp.heal")) {
                if (args.length == 0) {
                    player.setHealth(20);
                    player.sendMessage(Main.getPrefix() + "Du wurdest §ageheilt§7.");
                } else if (args.length == 1) {
                    Player target = Bukkit.getPlayer(args[0]);
                    if (target != null){
                        target.setHealth(20);
                        target.sendMessage(Main.getPrefix() + "Du wurdest §ageheilt§7.");
                        player.sendMessage(Main.getPrefix() + "Du hast §2" + target.getName() + " §ageheilt§7.");
                    } else
                        player.sendMessage(Main.getPrefix() + "§7Der Spieler §2" + args[0] + " §7ist §cnicht §7online.");
                }else
                    player.sendMessage(Main.getPrefix() + Main.use() + "§a/heal §7<Spieler>");
            } else
